...
<body>
    ...
    <div id="content">
        @yield('ban')
        <div id="left">
            <link rel="stylesheet" type="text/css" href="{{asset('css/front/left.css')}}" />
            <div id="subnav">
                <h3>
                    <img src="{{ asset('images/left.PNG') }}" />商品分类</h3>
                @foreach($typeTitle as $tyS)
                <h4><img src="{{ asset('images/login2.gif') }}" /><a href="{{route('front/recommend',array('typeid'=>$tyS->typeid,'titleShow'=>'精选男装'))}}">{{$tyS->typename}}</a></h4>
                @endforeach
            </div>
            <!--调用左部分的商品分类-->
            <div id="bottom">
                <!--导入数据库访问层代码-->
                <h3><img src="{{ asset('images/left.PNG') }}" />乐GO公告</h3>
                <ul>
                    @foreach($adverTitle as $adS)
                    <li style="text-align:center;"><a href="{{route('front/adver',array('id'=>$adS->id))}}" style="margin:0 auto;"> {{$adS->title}}</a></li>
                    @endforeach
                </ul>
            </div>
            <!--调用左部分的乐购公告-->
